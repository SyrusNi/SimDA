...
import trash_bin.job as classification
import submitit


def parse_args():
    classification_parser = classification.get_args_parser()
    parser = argparse.ArgumentParser("Submitit for SimDA", parents=[classification_parser])
    parser.add_argument("--mem_gb", default=0.1, type=float, help="Memory to request")
    ...
    parser.add_argument("--timeout", default=1, type=int, help="Duration of the job")
...
def main():
    ...
    executor.update_parameters(
        mem_gb=args.mem_gb, # first time 40G
        gpus_per_node=args.ngpus, # first time 1 card
        tasks_per_node=args.ntasks,  # one task per GPU # first time 1 task
        cpus_per_task=args.ncpus, # only use 1 cpu
        nodes=args.nodes, # 1 node to use
    ...
    executor.update_parameters(name="SimDA")
